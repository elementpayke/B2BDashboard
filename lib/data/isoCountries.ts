/** ISO 3166-1 alpha-2 countries for KYB / address pickers. */
...
export function searchCountries(query: string, limit = 12): IsoCountry[] {
  ...
  for (const c of ISO_COUNTRIES) {
    ...
    if (code === q || name.startsWith(q)) starts.push(c);
    else if (name.includes(q) || code.includes(q)) contains.push(c);
  }
