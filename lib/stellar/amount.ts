 ...
 */
export function parseStellarAmount(raw: string): { ok: true; amount: string } | { ok: false; error: string } {
  const trimmed = raw.trim().replace(/,/g, "").replace(/\.$/, "");
  if (!trimmed) {
    return { ok: false, error: "Enter an amount to send from your wallet." };
  }
  if (!/^\d+(\.\d+)?$/.test(trimmed)) {
  ...
  if (frac.length > MAX_STELLAR_DECIMALS) {
    return { ok: false, error: "Stellar amounts support at most 7 decimal places." };
  }
