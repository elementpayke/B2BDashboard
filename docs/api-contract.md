...
| Dashboard (Home) | `GET /v1/dashboard/summary`; `GET /v1/exchange-rates`; balances from `GET /v1/iban/accounts` + `GET /v1/entities/{id}/accounts` | Money in/out 30d, pending count from summary. Hero **Total balance** is **one** indicative FX total in a user-selected display currency (`SHOW IN`, persisted as `ep.displayCurrency.v1`). All wallets in the All/Fiat/Stablecoin filter are converted via USD-base rates (USDC/USDT peg to USD when no quote). A small USD equivalent sits under the figure when display ≠ USD. Currencies without a rate are excluded, never invented. Per-account native rows remain below. `totals.user_balance` (Privy) unused for the hero. |
