...
| Ops liquidity | `GET /v1/liquidity/stellar` | Admin-gated developer panel for the current Stellar stable swap book. Shows pairs plus `refreshed_at`; no trading UI on the merchant nav. |
| Collect Fund (stablecoin) | `GET …/deposit-instructions` (entity account) + ready FinancialAccount rails | **USDC on Element EVM rails** process into Stellar USDC home (CCTP). **EURC/USDT on Stellar only** convert via Aquarius. Fund modal copy must not imply EURC on Base/EVM or instant cross-rail credit. Intermediate CCTP rows appear as `processing` on `GET /v1/transactions` (`cctp_*`). |
...
| Add Account → Stablecoin Account | `GET /v1/entities`, `POST /v1/entities/{id}/accounts` | Opens a partner FinancialAccount (`asset_type: stablecoin`). **USDC on Base, Polygon, and Stellar**; allowlisted Stellar-only stables currently include **USDT** and **EURC**. Ethereum renders disabled. One account per currency × network. Uses the first linked entity; fails closed with a clear message when none exist. `display_name` is sent from the Account name field. |
...
| Wallets — Fund via Interac / open banking | `POST/GET …/payment-requests` | CAD Interac + EUR/GBP open banking when IBAN is `deposit_unavailable`. Never opens raw Nuvion hosts; shows scheme white-label pre-notice. |
| Wallets — Move (book transfer) | `POST …/book-transfers/preview`, `POST …/book-transfers` | Same-currency VA→VA via Mboka. |
...
| Area | Why |
|---|---|
| Deposit / Top up — **Stablecoin** tab | Address comes from the matching ready entity account (Stellar USDC = `G…`); EVM chips may show the dashboard-summary treasury. Network chips are Base / Polygon / Stellar only (`DEPOSIT_STABLECOIN_NETWORKS` — same creatable rails as Send). Mock `DEPOSIT_ADDRESSES` / Ethereum / Solana are not used. On allowlisted Stellar stables like USDT / EURC, the helper copy warns that inbound funds still credit as **USDC**. **Inbound visibility** is split by surface: Stellar USDC account detail reads recent on-chain wallet payments directly from Horizon (`/accounts/{G}/payments?order=desc&limit=&join=transactions`) and prefers ElementPay presentation only when a matching `tx_hash` already exists in `GET /v1/account-credits` / projected `GET /v1/transactions`. No client POST report-hash. If Horizon or Mboka credits are unavailable, fail closed (see Wired row + mapping notes). |
| Bulk-payout modal | When `STELLAR_BULK_PAYOUTS_ENABLED` is false we keep the local waitlist. When enabled, the modal submits real Stellar batch disbursements via `POST /v1/disbursements/stellar/preview` and `POST /v1/disbursements/stellar/confirm`, then shows the returned batch id + per-row statuses. **Send**, **Bulk**, and **Top up** entry points are KYB-gated like IBAN accounts until `kyb_status === "approved"`. |
...
4. **Send modal — Stablecoin tab**: ~~no backend endpoint for direct
   ...
   `POST /v1/entities/{id}/accounts` (USDC on Base/Polygon/Stellar) via
