...
| Dashboard (Home) | `GET /v1/dashboard/summary`; `GET /v1/exchange-rates`; balances from `GET /v1/iban/accounts` + `GET /v1/entities/{id}/accounts` | Money in/out 30d, pending count from summary. Hero **Balances** lists each account in its **own** currency (no FX-converted total, no `SHOW IN` / `ep.displayCurrency.v1`). All / Fiat / Stablecoin filters which rows appear. Missing partner balances render `—`. Live rates remain in the header/marquee for indicative FX. `totals.user_balance` (Privy) unused for the hero. |
...
| Balances (per-account + Home) | `GET /v1/iban/accounts`, `GET /v1/entities/{id}/accounts`, `GET /v1/exchange-rates` | Partner `balance.available` (fallback `current`) on fiat IBAN and entity stablecoin accounts, formatted through `lib/services/balances.ts`. A missing balance renders `—` — never invented. Convert and USDC Send refuse amounts above the known available. Home hero lists **native per-account balances** (no FX-converted total). Live `GET /v1/exchange-rates` ∪ summary `fx_rates` still power the rates marquee and Send quotes. |
